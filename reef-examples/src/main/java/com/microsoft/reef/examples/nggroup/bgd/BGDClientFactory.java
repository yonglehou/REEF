...
import com.microsoft.reef.examples.nggroup.bgd.parameters.BGDType;
...
public class BGDClientFactory {
  public static final BGDClient fromCommandLine(final String[] args) throws Exception {
    ...
    commandLine.registerShortNameOfClass(BGDType.class);
    ...
    final Injector injector = Tang.Factory.getTang().newInjector(configurationBuilder.build());
    final String bgdType = injector.getNamedInstance(BGDType.class);
    switch(bgdType) {
    case "FullBGD":
      configurationBuilder.bindImplementation(BGDClient.class, BGDClientImpl.class);
      break;
    default:
      throw new RuntimeException("Unsupported BGD type: " + bgdType + "Supported types: SimpleBGD|SimpleBGDLS|FullBGD");
    }

    return Tang.Factory.getTang().newInjector(configurationBuilder.build()).getInstance(BGDClientImpl.class);
  }
}
