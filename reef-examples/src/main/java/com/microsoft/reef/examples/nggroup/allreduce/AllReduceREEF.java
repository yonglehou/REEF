 ...
 */
@ClientSide
public class AllReduceREEF {
  private static final Logger LOG = Logger.getLogger(AllReduceREEF.class
    .getName());
  ...
  private static Configuration parseCommandLine(final String[] aArgs) {
    final JavaConfigurationBuilder cb = Tang.Factory.getTang()
  ...
  /**
   * copy the parameters from the command line required for the Client
   * configuration
   */
   ...
   *           if configuration injector fails.
   * @throws InjectionException
   *           if the Local.class parameter is not injected.
   */
  private static Configuration getRunTimeConfiguration() throws BindException {
    final Configuration runtimeConfiguration;
    if (local) {
      LOG.log(Level.INFO,
        "Running AllReduce example using nggroup API on the local runtime");
      runtimeConfiguration = LocalRuntimeConfiguration.CONF.set(
        LocalRuntimeConfiguration.NUMBER_OF_THREADS, NUM_LOCAL_THREADS).build();
    } else {
      LOG
        .log(Level.INFO, "Running AllReduce example using nggroup API on YARN");
  ...
  public static LauncherStatus runBGDReef(
    final Configuration runtimeConfiguration) throws BindException,
    InjectionException {

    final Configuration driverConfiguration = EnvironmentUtils
      .addClasspath(DriverConfiguration.CONF,
        DriverConfiguration.GLOBAL_LIBRARIES)
      .set(DriverConfiguration.ON_DRIVER_STARTED,
        AllReduceDriver.StartHandler.class)
      .set(DriverConfiguration.ON_EVALUATOR_ALLOCATED,
        AllReduceDriver.EvaluatorAllocatedHandler.class)
      .set(DriverConfiguration.ON_CONTEXT_ACTIVE,
        AllReduceDriver.ContextActiveHandler.class)
      .set(DriverConfiguration.ON_CONTEXT_CLOSED,
        AllReduceDriver.ContextCloseHandler.class)
      .set(DriverConfiguration.ON_TASK_FAILED,
        AllReduceDriver.FailedTaskHandler.class)
      .set(DriverConfiguration.DRIVER_IDENTIFIER, "AllReduceDriver").build();

    final Configuration groupCommServConfiguration = GroupCommService
      .getConfiguration();
    ...
    LOG.info(new AvroConfigurationSerializer()
      .toString(mergedDriverConfiguration));

    return DriverLauncher.getLauncher(runtimeConfiguration).run(
      mergedDriverConfiguration, JOB_TIMEOUT);
  }
