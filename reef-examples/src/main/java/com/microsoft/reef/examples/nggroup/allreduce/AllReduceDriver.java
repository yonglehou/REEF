...
@DriverSide
@Unit
public class AllReduceDriver {
  private static final Logger LOG = Logger.getLogger(AllReduceDriver.class
    .getName());
  ...
  final class StartHandler implements EventHandler<StartTime> {
    ...
    public void onNext(final StartTime startTime) {
      final int numEvals = AllReduceDriver.this.numberOfReceivers + 1;
      LOG.info("Requesting " + numEvals + " evaluators");
      AllReduceDriver.this.requestor.submit(EvaluatorRequest.newBuilder()
        .setNumber(numEvals).setMemory(1024).build());
  ...
  final class EvaluatorAllocatedHandler implements
    ...
    public void onNext(final AllocatedEvaluator allocatedEvaluator) {
      LOG.log(Level.INFO,
        "Submitting an id context to AllocatedEvaluator: {0}",
        allocatedEvaluator);
      try {
        final Configuration contextConfiguration =
          ContextConfiguration.CONF.set(
            ...
            "AllReduceContext-"
              + AllReduceDriver.this.numberOfAllocatedEvaluators
                .getAndDecrement()).build();
        allocatedEvaluator.submitContext(contextConfiguration);
      } catch (final BindException ex) {
        throw new RuntimeException(
          "Unable to setup Task or Context configuration.", ex);
      }
    }
  }
    ...
    private String contextId(final Configuration contextConf) {
      try {
        final Injector injector =
          Tang.Factory.getTang().newInjector(contextConf);
